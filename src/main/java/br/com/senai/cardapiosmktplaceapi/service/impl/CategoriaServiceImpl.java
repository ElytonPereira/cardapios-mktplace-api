...
public class CategoriaServiceImpl implements CategoriaService {
	...
	@Autowired
	private RestaurantesRepository restaurantesRespository;
	...
	public Categoria salvar(Categoria categoria) {
		...
		if(outraCategoria != null) {
			if(categoria.isPersistido()) {
				Preconditions.checkArgument(outraCategoria.equals(categoria), "O nome da categoria já esta em uso");
			}
	...
	@Override
	public void atualizaStatusPor(Integer id, Status status) {
		...
		Preconditions.checkArgument(categoriaEncontrada.getStatus() !=status, "O status já esta salvo para a categoria");
	...
	public Categoria buscarPor(Integer id) {
		...
		Preconditions.checkArgument(categoriaEncontrada.isAtiva(), "A categoria esta inativa");
	...
	public Categoria excluirPor(Integer id) {
		...
		Long qtdDeRestaurantesVinculados = restaurantesRespository.contarPor(id);
		Preconditions.checkArgument(qtdDeRestaurantesVinculados == 0, "Não é possivel remover, pois existem restaurantes vinculados");
